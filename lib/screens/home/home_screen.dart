...
void initState() {
  ...
  _controller = VideoPlayerController.asset('assets/images/b1.mp4')
    ..setLooping(true)
...
Widget build(BuildContext context) {
  return Scaffold(
    ...
    drawer: Drawer(
      backgroundColor: const Color.fromARGB(255, 190, 217, 242).withOpacity(0.8), // semi-transparent for glass effect
      child: ListView(
          ...
          DrawerHeader(
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: [const Color.fromARGB(255, 19, 68, 123), const Color.fromARGB(255, 34, 97, 149)], // Gradient header
              ),
            ),
            child: Center(
              child: Column(
            ...
            child: Column(
              ...
              children: [
                ...
                FutureBuilder<List<MusicTrack>>(
                  future: fetchLatestRecommendations(), // Ensure `userId` is available
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      ...
                      return _buildMusicRecommendations(snapshot.data!, context); // Music recommendations widget
                    }
                  },
                ),
              ],
...
String _tileText = "Log Emotions"; // Default text
  ...
  return Row(
    ...
    children: [
      Expanded(child: _buildNavButton(context, "Forum", Icons.forum, ForumHome(), Colors.cyan, false)),
      SizedBox(width: 10),
      Expanded(child: _buildNavButton(context, "Journal", Icons.book, JournalHome(), Colors.blueAccent, false)),
      SizedBox(width: 10),
      Expanded(child: _buildNavButton(context, "Lily", Icons.smart_toy, ChatbotScreen(), Colors.teal, true)), // Lily uses image
    ],
  );
}

Widget _buildNavButton(
    BuildContext context, String text, IconData icon, Widget page, Color color, bool isImageIcon) {
  return GestureDetector(
    onTap: () {
      ...
      child: BackdropFilter(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              isImageIcon 
                ? Image.asset('assets/images/lily.png', width: 30, height: 30) // Image for Lily
                : Icon(icon, color: Colors.white, size: 28), // Icons for other buttons
              SizedBox(height: 5),
              Text(
                text,
